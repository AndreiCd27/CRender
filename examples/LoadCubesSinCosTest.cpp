...
int cached0[sampleSize * 2][sampleSize * 2] = { {0} };
int cached1[sampleSize * 2][sampleSize * 2] = { {0} };

void graphFunction3D_t0(int x, int z) {
    //outputs y (UP)
    int y = (int)(sinf((float)x / 32.0f * glm::pi<float>()) * cosf((float)z / 32.0f * glm::pi<float>()) * 60.0f);
    cached0[x + sampleSize][z + sampleSize] = y;
}
void graphFunction3D_t1(int x, int z) {
    //outputs y (UP)
    int y = (int)(sinf((float)z * z / 512.0f * glm::pi<float>()) * cosf((float)x * x / 512.0f * glm::pi<float>()) * 60.0f);
    cached1[x + sampleSize][z + sampleSize] = y;
}
int graphFunction3D(int x, int z, const float t) {
    return t * cached0[x + sampleSize][z + sampleSize] + (1 - t) * cached1[x + sampleSize][z + sampleSize];
}
...
int main() {
    ...
    while (!engine->windowShouldClose()) {
        ...
        for (int x = -sampleSize; x < sampleSize; x++) {
            for (int z = -sampleSize; z < sampleSize; z++) {
                graphFunction3D_t0(x, z);
                graphFunction3D_t1(x, z);
